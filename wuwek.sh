...
GAME_ROOT_DIR="$HOME/Games/Wuthering Waves" # <<< EDIT THIS PATH >>>
...
install_dxvk_if_needed() {
  ...
  if [[ -f "$flag_file_path" ]]; then
    echo "Info [Game Mode]: DXVK flag file found. Skipping DXVK copy."
    return 0
  fi
  echo "Info [Game Mode]: DXVK flag file not found. Attempting DXVK installation/update..."
  local dxvk_source_x64="$dxvk_source_path/x64"
  local dxvk_source_x32="$dxvk_source_path/x32"
  if [[ ! -d "$dxvk_source_path" || ! -d "$dxvk_source_x64" || ! -d "$dxvk_source_x32" ]]; then
    echo "Error [Game Mode]: DXVK Source ('$dxvk_source_path') or subfolders (x64/x32) not found." >&2
    return 1
  fi
  local target_system32="$prefix_path/drive_c/windows/system32"
  local target_syswow64="$prefix_path/drive_c/windows/syswow64"
  mkdir -p "$target_system32" "$target_syswow64"
  local all_copied=true
  echo "Info [Game Mode]: Copying x64 DXVK DLLs..."
  cp -v "$dxvk_source_x64/"{d3d9,d3d10core,d3d11,dxgi}.dll "$target_system32/" || all_copied=false
  echo "Info [Game Mode]: Copying x32 DXVK DLLs..."
  cp -v "$dxvk_source_x32/"{d3d9,d3d10core,d3d11,dxgi}.dll "$target_syswow64/" || all_copied=false
  if [[ "$all_copied" == "true" ]]; then
    echo "Info [Game Mode]: DXVK DLLs copied successfully."
    touch "$flag_file_path"
    echo "Info [Game Mode]: Created flag file '$flag_file_path'."
    return 0
  else
    echo "Error [Game Mode]: Failed to copy DXVK DLLs."
    return 1
  fi
}
...
install_vkd3d_if_needed() {
  ...
  local flag_file_path="$prefix_path/$VKD3D_FLAG_FILE_NAME"
  if [[ "$ENABLE_VKD3D" != "true" || -z "$vkd3d_source_path" ]]; then return 0; fi
  if [[ -f "$flag_file_path" ]]; then
    echo "Info [Game Mode]: VKD3D-Proton flag file found. Skipping VKD3D copy."
    return 0
  fi
  ...
  mkdir -p "$target_system32" "$target_syswow64"
  local all_copied=true
  local vkd3d_dlls_to_copy=("d3d12.dll" "d3d12core.dll")
  echo "Info [Game Mode]: Copying x64 VKD3D-Proton DLLs (${vkd3d_dlls_to_copy[*]})..."
  for dll in "${vkd3d_dlls_to_copy[@]}"; do if [[ -f "$vkd3d_source_x64/$dll" ]]; then cp -v "$vkd3d_source_x64/$dll" "$target_system32/" || all_copied=false; else echo "Warning [Game Mode]: VKD3D x64 DLL not found in source: $dll" >&2; fi; done
  echo "Info [Game Mode]: Copying x32 VKD3D-Proton DLLs (${vkd3d_dlls_to_copy[*]})..."
  for dll in "${vkd3d_dlls_to_copy[@]}"; do if [[ -f "$vkd3d_source_x32/$dll" ]]; then cp -v "$vkd3d_source_x32/$dll" "$target_syswow64/" || all_copied=false; else echo "Warning [Game Mode]: VKD3D x32 DLL not found in source: $dll" >&2; fi; done
  if [[ "$all_copied" == "true" ]]; then
    echo "Info [Game Mode]: VKD3D-Proton DLLs copied successfully."
    touch "$flag_file_path"
    echo "Info [Game Mode]: Created flag file '$flag_file_path'."
    return 0
  else
    echo "Error [Game Mode]: Failed to copy one or more VKD3D-Proton DLLs."
    return 1
  fi
}
...
run_patcher() {
  ...
  if ! command -v bbe &>/dev/null; then
    echo "Error [Patcher Mode]: bbe is not installed." >&2
    return 1
  fi
  local version_dir=$(find "$base_dir_resolved" -maxdepth 1 -type d -name "[0-9]*.[0-9]*.[0-9]*.[0-9]*" | sort -V | tail -n1)
  if [[ -z "$version_dir" ]]; then
    echo "Error [Patcher Mode]: No version directory found in '$base_dir_resolved'." >&2
    return 1
  fi
  ...
  if [[ ! -f "$target_dll_path" ]]; then
    echo "Error [Patcher Mode]: Target DLL not found: '$target_dll_path'" >&2
    return 1
  fi
  if ! grep -a -q $'\x12AllowsTransparency' "$target_dll_path"; then
    echo "Info [Patcher Mode]: '$PATCHER_TARGET_DLL' already patched or pattern not found."
    return 0
  fi
  local backup_dll_path="${target_dll_path}.bak"
  echo "Info [Patcher Mode]: Backing up '$target_dll_path'..."
  mv "$target_dll_path" "$backup_dll_path" || {
    echo "Error: Failed backup."
    return 1
  }
  echo "Info [Patcher Mode]: Applying patch using bbe..."
  bbe -e "s/\x12AllowsTransparency/\x09IsEnabled\x1bA\x00\x03AAAAA/" "$backup_dll_path" >"$target_dll_path" || {
    echo "Error: bbe patch failed. Restoring."
    mv "$backup_dll_path" "$target_dll_path"
    return 1
  }
...
run_launcher() {
  ...
  if [[ ! -d "$prefix_parent_dir" ]]; then
    echo "Error [Launcher Mode]: Parent dir for prefix ('$prefix_parent_dir') missing." >&2
    return 1
  fi
  if [[ ! -f "$launcher_exe_resolved" ]]; then
...
run_game() {
  ...
  local prefix_parent_dir=$(dirname "$prefix_resolved")
  if [[ ! -d "$prefix_parent_dir" ]]; then
    echo "Error [Game Mode]: Parent dir for prefix ('$prefix_parent_dir') missing." >&2
    return 1
  fi
  if [[ ! -f "$launcher_exe_resolved" ]]; then
    echo "Error [Game Mode]: Jadeite launcher EXE not found: '$launcher_exe_resolved'" >&2
    return 1
  fi
  ...
  if [[ "$effective_gamemode" == "true" ]] && ! command -v gamemoderun &>/dev/null; then
    echo "Warning [Game Mode]: GameMode (-g) needs 'gamemoderun'. Disabling." >&2
    effective_gamemode=false
  fi
  if [[ "$effective_mangohud" == "true" ]] && ! command -v mangohud &>/dev/null; then
    echo "Warning [Game Mode]: MangoHud (-m) needs 'mangohud'. Disabling." >&2
  ...
  if [[ "$ENABLE_DXVK" == "true" ]]; then
    ...
    echo "Info [Game Mode]: Enabling DXVK overrides ($current_overrides)."
  fi
  if [[ "$SELECTED_DX_MODE" == "dx12" && "$ENABLE_VKD3D" == "true" ]]; then
    local vkd3d_overrides="d3d12,d3d12core=n"
    echo "Info [Game Mode]: Enabling VKD3D-Proton overrides ($vkd3d_overrides) for DX12."
    [[ -n "$current_overrides" ]] && current_overrides+=";"
    current_overrides+="$vkd3d_overrides"
  elif [[ "$SELECTED_DX_MODE" == "dx12" && "$ENABLE_VKD3D" != "true" ]]; then echo "Warning [Game Mode]: DX12 mode selected, but ENABLE_VKD3D not true." >&2; fi
  if [[ -n "$ADDITIONAL_OVERRIDES" ]]; then
    echo "Info [Game Mode]: Adding custom overrides ($ADDITIONAL_OVERRIDES)."
    [[ -n "$current_overrides" ]] && current_overrides+=";"
    current_overrides+="$ADDITIONAL_OVERRIDES"
  fi
  [[ -n "$current_overrides" ]] && export WINEDLLOVERRIDES="$current_overrides" || unset WINEDLLOVERRIDES
  if [[ "$ENABLE_DXVK" == "true" && -n "$DXVK_HUD_OPTIONS" ]]; then
    export DXVK_HUD="$DXVK_HUD_OPTIONS"
    echo "Info [Game Mode]: Enabling DXVK HUD ($DXVK_HUD_OPTIONS)."
  else unset DXVK_HUD; fi
...
while [[ $# -gt 0 ]]; do
  ...
  -g | --gamemode)
    ...
    ;;
  --dx11)
    SELECTED_DX_MODE="dx11"
    shift
    ;;
  --dx12)
    SELECTED_DX_MODE="dx12"
    shift
    ;;
  -h | --help) show_help ;;
  *)
    EXTRA_GAME_ARGS_ARRAY+=("$1")
    shift
    ;;
  esac
done
...
if [[ -n "$WINE_CMD" ]]; then
  ...
  if [[ -z "$WINE_TO_USE" ]]; then
    echo "Error: WINE_CMD empty and system 'wine' not found." >&2
    exit 1
  fi
fi
...
game) run_game ;;
*)
  echo "Error: Invalid mode '$SCRIPT_MODE'. Use 'patch', 'launcher', or 'game'." >&2
  show_help
  exit 1
  ;;
esac
